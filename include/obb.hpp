 ...
 *
 * Stores a quaternion `rotation`, a world-space `center` and `halfExtents`.
 * The helper functions below project OBBs onto axes and implement SAT
 * collision tests used by the physics system.
 */
struct OBB
{
...
/**
 * @brief Extract orthonormal local axes from an OBB's quaternion.
 *
 * @param obb Source OBB
 * @param right Out: local X axis
 * @param up Out: local Y axis
 * @param forward Out: local Z axis
 */
...
/**
 * @brief Calculate the eight world-space corners of an OBB.
 *
 * @param obb Source OBB
 * @param corners Out: array of 8 corner positions
 */
...
/**
 * @brief Draw the OBB wireframe using `DrawLine3D` for debugging.
 */
...
/**
 * @brief Test whether a world-space point is contained by the OBB.
 */
...
/**
 * @brief Project an AABB onto an axis (used by SAT tests).
 */
...
/**
 * @brief Project an OBB onto an axis (efficient version using local axes).
 */
...
/**
 * @brief SAT test for collision between an AABB and an OBB.
 */
...
/**
 * @brief Detailed collision result used by OBB-vs-OBB SAT checks.
 *
 * `penetration` and `normal` represent the Minimum Translation Vector
 * (MTV) required to separate the two boxes.
 */
...
/**
 * @brief Compute collision info (MTV) between two OBBs using SAT.
 *
 * Returns a CollisionResult with `collided==false` if a separating axis
 * was found; otherwise `penetration` contains the minimum overlap and
 * `normal` the corresponding axis (pointing from B to A).
 */
...
/**
 * @brief Ray vs OBB intersection test. Transforms ray to local space and
 * performs slab tests.
 */
...
/**
 * @brief Sphere vs OBB collision test using closest-point clamping.
 */
...
inline bool CheckLineSegmentVsOBB(Vector3 start, Vector3 end, float radius, const OBB *obb, float *outDistance = nullptr)
{
